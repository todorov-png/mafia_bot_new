...
export function userRegistrationBtn(urlBot, chatID) {
    return Markup.inlineKeyboard([Markup.urlButton('🕵️‍♂️ Присоединиться 🕵️‍♂️', urlBot + '?start=' + chatID)]);
}

export function voteDay(urlBot) {
    return Markup.inlineKeyboard([Markup.urlButton('🗳 Голосовать 🗳', urlBot)]);
}

export function goToBot(urlBot) {
    return Markup.inlineKeyboard([Markup.urlButton('🚶 Перейти к боту 🚶', urlBot)]);
}
...
export function newGame() {
    return Markup.inlineKeyboard([Markup.callbackButton('Начать новую игру!', 'newgame')]);
}
