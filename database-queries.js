...
export async function getDataCheckingLoggedUser(chatID) {
    return await mongobd.Chats.findOne({ chatID: chatID }, { listOfUser: true, _id: false });
}
...
export async function updateDataSendMessageRegistration(chatID, messageID, time) {
...
export async function clearCounterActiveRoles(chatID) {
    await mongobd.Chats.updateOne({ chatID: chatID }, { 'dataGame.counterActiveRoles': 0 });
}
...
export async function updateDataPlayers(chatID, players) {
    await mongobd.Chats.updateOne({ chatID: chatID }, { players: players });
}

export async function updateDataAutoUpdateTitleChat(chatID, title) {
    await mongobd.Chats.updateOne({ chatID: chatID }, { title: title });
}

export async function updateDataAutoUpdateIDChat(chatID, newChatID) {
    await mongobd.Chats.updateOne({ chatID: chatID }, { chatID: newChatID });
}
...
export async function updateNameUser(chatID, userID, nameUser, userName) {
    await mongobd.Chats.updateOne(
        { chatID: chatID },
        { 'listOfUser.$[index].name': nameUser, 'listOfUser.$[index].userName': userName },
...
export async function clearAllVoticeDay(chatID) {
    await mongobd.Chats.updateMany(
        { chatID: chatID },
        {
...
export async function getIdMedia(name) {
    return await mongobd.Media.findOne(
        { fileName: name },
        {
            fileId: true,
            data: true,
...
export async function setMedia(obj) {
    return await mongobd.Media.findOneAndUpdate({ fileName: obj.fileName }, obj, {
        upsert: true,
        setDefaultsOnInsert: true,
    });
}

export async function getChatIdActiveGame() {
    return await mongobd.Chats.find(
        { messageID: { $ne: 0 } },
        {
            chatID: true,
            messageID: true,
...
export async function getChatIdCompletedGame() {
    return await mongobd.Chats.find(
        { messageID: 0 },
        {
            chatID: true,
